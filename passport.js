...
app.all('*', (req, res, next) => {
    ...
    const whiteOriginDomain = '.moriarty.com'
...
app.post('/login', async (req, res, next) => {
    ...
    try {
        ...
        res.setHeader(
            'Set-Cookie',
            `token=${newToken};domain=moriarty.com;max-age=${tokenConfig.expiresIn};httpOnly`)
...
app.post('/register',async(req,res,next)=>{
    ...
    try {
        ...
        if (!dataValues) {
            let regisValue = await userModel.create({
                id:uuidV4(),
                username:username,
                pass:passwordMD5(password)
            })
            console.log(regisValue)
            return res.json({
                code:0,
                msg:'success'
            })
